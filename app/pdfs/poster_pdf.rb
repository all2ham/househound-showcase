class PosterPdf < Prawn::Document
  ...
  def initialize(listing, view)
    ...
    @view = view
    float do
      image "#{Rails.root}/app/assets/images/listings/posterbg.jpg",:width => 612, :height => 792
    end
    pdf_address
    pdf_mainimage
    pdf_smallimages
  ...
  def pdf_price
    ...
    fill_color "FFFFFF"
    text_box "Price: #{number_to_currency(@listing.price)}", :align=>:center,:at => [295,437],:width=>300
  end

  def pdf_bedcount
    font("Helvetica",:size =>28)
    fill_color "FFFFFF"
    text_box "#{@listing.bedroom_count}", :align=>:center,:at => [15,437],:width=>70
  end

  def pdf_bathcount
    font("Helvetica",:size =>28)
    fill_color "FFFFFF"
    text_box "#{@listing.bathroom_count}", :align=>:center,:at => [155,437],:width=>70
  end
  ...
  def pdf_agent_pic
    ...
    if @agent_profile.photo
      ...
      image open(agentpic.photo.url), :at=>[29,121],:width => 78, :height => 94
    end
  end
  ...
  def dummy_image
    float do
      image "#{Rails.root}/app/assets/images/listings/listing_photo_placeholder.jpeg", :at=>[170,674],:width => 275, :height => 205
    end
  end
end